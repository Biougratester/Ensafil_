        ...
        for fillier in list(branches.keys()):
            ...
            for i in range(1,40):
                if request.json["ratings"][str(i)] !=None:
                    try:
                        order += (float(request.json["ratings"][str(i)]))*float(branches[fillier][i])
                    except:
                        order+=0
                else:
                    order += 0
            order_list[fillier]=order
        order_list_=dict()
        for i in range(7):
            key = list(order_list.keys())[i]
            order_list_[key]=1
            range_list = list(range(7))
            range_list.remove(i)
            for j in range_list:
                key_ = list(order_list.keys())[j]
                if order_list[key]<order_list[key_]:
                    order_list_[key]+=1
        context = []
        sorted_order_list = sorted(order_list_.items(), key=lambda x:x[1])
        for f,o in sorted_order_list:
            context.append({"fill":filliers[f],"order":o})
        dbconnector("w",fullname = request.json["fullname"],ratings = request.json["ratings"],order = order_list_)
